  ...
  el.innerHTML = `
    <div class="editor-card">
      ...
      <div class="field">
        ...
        <div class="severe-note" data-ref="severe" ${marker.intensity < 8 ? 'hidden' : ''}>
          Severe pain — if it came on suddenly or is the worst you've had, treat it as urgent.
        </div>
      </div>
...
export function renderPointsList(el, countEl, ctx) {
  ...
  el.innerHTML = markers.map(m => {
    ...
    const meta = [band.label, depthById(m.depth).short, q?.label, m.note ? '📝' : '']
      .filter(Boolean).join(' · ');
    return `
